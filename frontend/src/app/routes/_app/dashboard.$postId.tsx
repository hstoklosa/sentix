...
function PostComponent() {
  ...
  useEffect(() => {
    ...
    if (symbolsChanged) {
      ...
      if (symbolsToRemove.length > 0) unsubscribeFromSymbols(symbolsToRemove);

      const symbolsToAdd = symbols.filter(
        (symbol) => !previousSymbolsRef.current.includes(symbol)
      );

      if (symbolsToAdd.length > 0) subscribeToSymbols(symbolsToAdd);
          ...
          <div className="flex flex-col h-full overflow-hidden">
            <div className="flex items-center gap-2 px-2.5 py-2 border-b border-border">
              ...
              <div className="ml-auto flex items-center gap-3">
                <button
                  aria-label={
                    post.is_bookmarked ? "Remove bookmark" : "Add bookmark"
                  }
